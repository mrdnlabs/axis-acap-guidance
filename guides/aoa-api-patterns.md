...
- **`ax_event_handler_subscribe` succeeds even if the AOA scenario does not exist.** The AXEvent handler subscribes on topic name, not on a live event source. `ax_event_handler_subscribe(handler, kv_set, &sub_id, on_line_crossing, NULL, &err)` returns `true` for `tnsaxis:CameraApplicationPlatform/ObjectAnalytics/Device1Scenario999` even if scenario 999 was never created. The callback simply never fires and there is no error. Guard against this two ways in production:
  1. Verify the scenario exists by calling `getConfiguration` right after subscribe, and log an `LOG_WARNING` if the configured `AoaScenarioId` isn't in the returned list.
  2. Emit a periodic health check — if no AOA event has been received in N minutes and the scenario is expected to be active, log `LOG_WARNING`. This catches the "someone deleted the scenario" case that no init-time check will catch.
